...
const DAVA::String TANK_MODEL_PATH = "~res:/3d/Tanks/German/Leopard1.sc2";
}
...
void ViewSceneScreen::AddMenuControl()
{
    ...
    selectSceneMenuItem = mainSubMenu->AddActionItem(L"Select scene", DAVA::Message(this, &ViewSceneScreen::OnButtonSelectFromRes));
...
void ViewSceneScreen::OnSceneSelected(const DAVA::FilePath& pathToFile)
{
    menu->SetEnabled(true);
    scenePath = pathToFile;
    data.settings.SetLastOpenedScenePath(scenePath);
...
void ViewSceneScreen::OnButtonSelectFromRes(DAVA::BaseObject* caller, void* param, void* callerData)
{
    ...
    fileSystemDialog->SetCurrentDir("~res:/3d/Maps/");
